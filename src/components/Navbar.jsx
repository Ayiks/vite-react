...
const Navbar =() => {
    ...
    return (
        <nav className= {`${styles.paddingX} w-full flex items-center py-2 fixed top-0 z-20 bg-flashWhite sm:opacity-[0.97] xxs:h-[12vh]`}>
           ...
           >
            <h3
            className="text-eerieBlack font-extrabold uppercase text-[21px]"
            >Eastern Tech Hub.</h3>
            ...
            <ul className="list-none hidden sm:flex flex-row gap-14 mt-2 ">
                {navLinks.map((nav) => (
                    <li
                    key={nav.id}
                    className={`${active == nav.title ? 'text-french': 'text-eerieBlack'} hover:text-taupe text-[21px] font-medium uppercase tracking-[3px] cursor-pointer nav-links`}
                            ...
                            <ul 
                                ...
                                {navLinks.map((nav)=> (
                                    ...
                                    className={`${active == nav.title ? 'text-french': 'text-eerieBlack'} 
                                        text-[15px] font-mova uppercase tracking-[1px] cursor-pointer`}
                                    onClick={()=> {
                                        setToggle(!toggle);
                                        setActive(nav.title);
                                    }}
